...
module TB_toplevel;
    ...
    always @ (posedge clk50) begin
        ...
        else if (LED == 16'h0BAD) begin // any other non-zero value is an error (the test program should only write zero to the LEDs, by design)
            $display("[FAIL!!!] At time %t, LED state: %h", $time, LED);
            fail_count++;
        end
    end

    always @ (posedge (LED == 16'h900D)) begin
        $display("          Got signal '900D' on LEDs at time %t, assertion passed", $time);
    end

    initial begin
        forever begin
            @(posedge clk50);
            $display("Time: %50t | imem raddr: %8h | imem rdata: %8h | GPIO1: %4h | LED: %4h", $time, dut.instr_if.addr, dut.instr_if.rdata, dut.gpio_out1, LED);
        end
    end
endmodule
